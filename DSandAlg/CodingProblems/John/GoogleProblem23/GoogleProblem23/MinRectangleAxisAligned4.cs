...
namespace GoogleProblem23
{
    public class MinRectangleAxisAligned4
    {
        ...
        private static int RectangleArea(int[] rect, int[][] points)
        {
            ...
            // SS: to check whether 4 points for a rectangle,
            // 1. opposite sides have the same length
            // 2. the diagonals have the same length

            var d01 = p01x * p01x + p01y * p01y;
            var d23 = p23x * p23x + p23y * p23y;
            if (d01 != d23)
            {
                return int.MaxValue;
            }

            var d12 = p12x * p12x + p12y * p12y;
            var d30 = p30x * p30x + p30y * p30y;
            if (d12 != d30)
            {
                return int.MaxValue;
            }
            ...
            var area = Math.Abs(p23x * p30y - p23y * p30x);
            if (area == 0)
            {
                // SS: all points collinear
                return int.MaxValue;
            }
            return area;
        }
